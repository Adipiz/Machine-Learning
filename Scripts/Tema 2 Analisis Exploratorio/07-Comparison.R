...
# Comparación entre diferentes variables de una data set ------------------
...
library(beanplot)
par(mfrow = c(1,1))
beanplot(data$cnt ~ data$season, col = c("blue", "red", "yellow", "black"))
...
bwplot(cnt~weathersit, data = data,
       ...
       par.settings = list(
         box.rectangle = list(fill = c("red", "yellow", "green"))))
